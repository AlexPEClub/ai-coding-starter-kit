...
| PROJ-41 | Fahrer — Fahrt bearbeiten (Fahrer/Datum/Notiz + Änderungsverlauf) | 🟡 In Progress — Frontend + Backend in einem Durchgang umgesetzt (analog PROJ-21, kein separater `/backend`-Schritt nötig): Migration (`tours.notiz`, neue Tabelle `tms.tour_aenderungen`) live angewendet, inkl. Hotfix-Migration für fehlende `service_role`-GRANTs (gleiches Muster wie PROJ-29). Neue Server Actions `bearbeiteFahrt()`/`getFahrtAenderungen()`, neuer Bearbeiten-Dialog, jede Stopp-Zeile antippbar. Echter Bug gefunden+gefixt: `router.refresh()` nach Speichern ergänzt, da `revalidatePath()` allein den Client nicht nachlädt. Alle Änderungen (Fahrer/Datum/Notiz/Verlauf) live gegen Produktivdaten über direkte DB-Abfragen verifiziert, danach zurückgesetzt. Neue E2E-Suite `tests/PROJ-41-fahrt-bearbeiten.spec.ts` (6 Tests, jeder einzeln grün) — Batch-Lauf auf dem akut speicherknappen Dev-Host nicht durchgängig stabil (Umgebungsbefund, siehe Spec). Details in `features/PROJ-41-fahrt-bearbeiten.md`. Nächster Schritt: `/qa PROJ-41` | 2026-08-02 |
