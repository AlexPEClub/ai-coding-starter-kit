...
| PROJ-29 | Wissensbasis (KI-Content-Fundament) | 🟠 In Review — Backend gebaut & live angewendet (Migration, Storage-Policies, FTS, RPCs, Server Actions auf echtes Supabase umgestellt). QA (2026-07-27): Code-/Schema-Review statt Live-Browser-Test (Sandbox-Umgebung kommt nicht an echte `.env.local`-Werte, auch nach Rücksprache mit dem User); echte PDF-Extraktion gegen das Leitz-Anwenderlexikon direkt verifiziert (163k Zeichen), Fehlerpfad gegen kaputte Datei bestätigt, RLS-Policies unabhängig per DB-Query geprüft. 9/10 Akzeptanzkriterien erfüllt (1 offen: Leitz-Lexikon-Erstupload steht noch aus, macht der User selbst), 2 Low-Bugs gefunden und noch am selben Tag gefixt (unnötiger Nachladeaufwand bei Tag-Edits, unerreichbare „nicht gefunden"-Meldung beim Löschen). Bleibt „In Review" bis Live-Test nachgeholt oder bewusst übersprungen wird | 2026-07-27 |
...
| PROJ-34 | Werkzeug-/Auftrags-Fundament + Fahrer-Auftragserfassung + Wareneingang | 🟢 Approved — Herzstück von TMS 2.0, Teil 1/6. Zwei QA-Runden (`features/PROJ-34-werkzeug-auftrag-fundament.md`, QA Test Results): Runde 1 fand 4 Bugs (26/28 AC bestanden), alle direkt gefixt — **BUG-1 (High, PostgREST-Filter-Injection in `resolveQrCode()`)**, BUG-2 (Fehlermeldung nennt jetzt Kundennamen), BUG-3 (try/catch/finally in allen Dialog-Handlern), BUG-4 (Begleit-QR bewusst nach PROJ-35 verschoben). Runde 2 hat alle 4 Fixes unabhängig re-verifiziert (BUG-1 zusätzlich mit 4 weiteren Angriffsvarianten, alle schlagen fehl), keine neuen Bugs. 198 Unit-Tests + Build + Lint grün. Browser-E2E vierfach versucht, scheiterte durchgehend an Host-Ressourcen-Engpässen (nicht an der App, inkl. `frame detached`-Fehler unter Speicherdruck) — `tests/PROJ-34-werkzeug-auftrag-fundament.spec.ts` bleibt als Regressions-Suite, sollte vom User lokal/CI einmal verifiziert werden. Keine Critical/High-Bugs mehr offen. Nächster Schritt: `/deploy PROJ-34` | 2026-07-28 |
