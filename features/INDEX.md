...
| PROJ-21 | Fahrer — Tourenliste (nur Anzeige) | 🟢 Approved — QA bestanden: 8/9 Acceptance Criteria live verifiziert (Chromium, gegen Produktivdaten, kein Staging vorhanden), 1/9 indirekt über identischen Code-Pfad abgedeckt. 2 Bugs gefunden — **BUG-1 (Medium, Server Actions in `fahrten.ts` prüften nur Login statt Rolle `fahrer`/`admin`) noch am selben Tag gefixt** (neue `pruefeFahrerZugriff()`-Prüfung in allen drei Aktionen, lint/build/Unit-Tests/E2E-Suite danach erneut grün, keine Regression); BUG-2 (Low, A11y — Fahrer-Filter-Dropdown ohne zugänglichen Namen) bleibt offen als Polish-Punkt. Mobile Safari (WebKit) auf diesem ressourcenknappen Host nicht abschließend testbar (Login submittete wiederholt nicht unter WebKit, während Chromium 8/8 zuverlässig lief) — als Umgebungs-, nicht Produktbefund eingeordnet; vor `/deploy` erneut versuchen. Unit-Tests (`fahrten-helpers.test.ts`, 4/4) + E2E-Suite (`tests/PROJ-21-fahrer-tourenliste.spec.ts`, 8/8) neu geschrieben. Regression (Nav, `/wareneingang`, `/dashboard`) grün. Details in `features/PROJ-21-fahrer-tourenliste.md`. Nächster Schritt: `/deploy PROJ-21` (BUG-2 optional vorher/danach beheben) | 2026-08-01 |
