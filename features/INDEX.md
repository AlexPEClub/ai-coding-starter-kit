...
| PROJ-42 | Routenberechnung für Touren (Geoapify) | 🟢 Approved — Re-QA nach Bugfixes bestanden: BUG-1 (High, Autorisierungslücke `pickup-tours.ts`) und BUG-2 (Low, keine Rate-Begrenzung der Neuberechnung) beide gefixt und re-verifiziert — BUG-1 live bestätigt (neuer Test `tests/PROJ-42-routenberechnung.spec.ts`: Admin-Account sieht Bearbeiten-Zugriff auf der Kundendetailseite, Dialog funktioniert; Ablehnungsfälle unit-getestet), BUG-2 live bestätigt (Server-Log zeigt Cooldown bei zwei unabhängigen Testläufen: „Cooldown aktiv (zuletzt vor 3s ausgelöst)"). Komplette PROJ-21/PROJ-41/PROJ-42-E2E-Suite (17 Tests) grün, 414 Unit-Tests grün, `npm run build`/`tsc`/`eslint` fehlerfrei. 9/13 Acceptance Criteria live oder per Code-Review verifiziert, 4/13 nur per Unit-Test (blockiert durch fehlenden `GEOAPIFY_API_KEY`/Backfill-Lauf — kein Blocker für Approval, da Fehlschlag-Verhalten dafür bereits live bewiesen ist). Details/volle QA-Ergebnisse in `features/PROJ-42-routenberechnung.md`. Baut auf PROJ-21/PROJ-41 auf. Nächster Schritt: `/deploy` | 2026-08-02 |
