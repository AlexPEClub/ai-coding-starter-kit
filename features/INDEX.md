...
| PROJ-35 | Arbeitsvorbereitung — Werkzeugkategorien & Pfade (Stammdaten) | ✅ Deployed — Admin-Stammdaten: Ober-/Unterkategorien, globales Geometrie-Parameter-Register, automatische Serviceartikel-Zuordnung über Preisstaffel (zweistufig: Checkbox-Kandidatenauswahl, dann Wertebereich je Kandidat), wiederverwendbare Pfade (Ort im Betrieb/extern) inkl. manueller Dienstleister-Anlage (`partner_type='supplier'`). QA-Runde 1 fand 6 Bugs (2 High: Serviceartikel-Neuanlage/`products.id` ohne Default, PostgREST-Filter-Injection aus PROJ-28 neu erreichbar; 2 Medium: "Einsatzbereit"-Prüfung, Parameter-Register ohne Deaktivierung; 2 Low), auf User-Wunsch alle 6 gefixt und einzeln gegen die Live-DB re-verifiziert. Am 2026-07-30 live nach https://tms.gudel-werkzeuge.de deployed (`./scripts/deploy.sh PROJ-35`, Post-Deploy-Smoke grün im 3. Anlauf, `/verwaltung/werkzeugkategorien` manuell verifiziert, keine Fehler in Container-Logs). Kein Staging vorhanden — direkt gegen Produktion deployed. Baut auf PROJ-34/28 auf. Teil 2a/7. Details in `features/PROJ-35-werkzeugkategorien-pfade-stammdaten.md`. Nächster Schritt: `/write-spec PROJ-40` (AV-Workflow) | 2026-07-30 |
