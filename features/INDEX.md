...
| PROJ-41 | Fahrer — Fahrt bearbeiten (Fahrer/Datum/Notiz + Änderungsverlauf) | 🟢 Approved — QA bestanden: 9/9 Acceptance-Criteria-Gruppen live verifiziert (1 davon teilweise per Code-Review, da noch keine Live-Fahrten mit Status unterwegs/angekommen/problem existieren). E2E-Suite `tests/PROJ-41-fahrt-bearbeiten.spec.ts` 6/6 grün, PROJ-21-Regression 8/8 weiterhin grün. Security: Rollen-Check (`pruefeFahrerZugriff()`, session-basiert) strukturell bestätigt; ein Live-Exploit-Versuch war methodisch nicht schlüssig (eigenes Payload-Format traf die Funktion nie, ehrlich als Einschränkung vermerkt statt fälschlich als "bestätigt" ausgegeben). 1 Bug gefunden — **BUG-1 (Low, Datenintegrität) noch am selben Tag gefixt**: `fahrerId` wird jetzt serverseitig gegen die echte Fahrerliste geprüft (`.contains("roles",["fahrer"])`, gleiche Abfrage wie `listFahrerOptionen()`), nicht mehr nur auf Existenz in `auth.users`; danach erneut lint/build/E2E-Suite grün, keine Regression. Alle Live-Test-Änderungen an echten Stopps (Rhehag, Tönnissen, Gallhoff) zurückgesetzt, angesammelte Test-Verlaufseinträge wieder gelöscht. Details in `features/PROJ-41-fahrt-bearbeiten.md`. Nächster Schritt: `/deploy PROJ-41` | 2026-08-02 |
