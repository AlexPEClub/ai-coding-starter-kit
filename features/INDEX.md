...
| PROJ-42 | Routenberechnung für Touren (Geoapify) | ✅ Deployed — Live seit 2026-08-02 unter https://tms.gudel-werkzeuge.de (`./scripts/deploy.sh PROJ-42`, Post-Deploy-Smoke grün im 1. Anlauf, keine Fehler in Container-Logs). Migration gegen die Produktions-DB angewendet (sicherer No-Op). QA zuvor bestanden inkl. Re-Verifikation zweier gefixter Bugs (BUG-1 High: fehlende Rollenprüfung in pickup-tours.ts; BUG-2 Low: keine Rate-Begrenzung der Neuberechnung) — beide live + unit-getestet. **Bewusst ohne `GEOAPIFY_API_KEY`/Depot-Koordinaten deployed** (User-Entscheidung: später nachtragen) — Routenberechnung bleibt bis dahin inaktiv, aber sicher (Trigger schlagen geloggt fehl, Touren-Liste zeigt weiter den bisherigen Datums-Fallback), kein zweiter Deploy nötig sobald die 3 Env-Variablen gesetzt sind + `npm run backfill:routen` einmalig läuft. Kein dediziertes `tests/deploy/PROJ-42-*.spec.ts` (Folge-Polish empfohlen, analog PROJ-21/41). Git-Tag `v1.42.0-PROJ-42`. Details in `features/PROJ-42-routenberechnung.md`. Baut auf PROJ-21/PROJ-41 auf. Nächster Schritt: User trägt Geoapify-Env-Vars nach, dann `npm run backfill:routen` | 2026-08-02 |
