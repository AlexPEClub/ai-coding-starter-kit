...
| PROJ-29 | Wissensbasis (KI-Content-Fundament) | 🟡 In Progress — Backend gebaut & live angewendet: Migration `20260726100000_PROJ-29_wissensbasis_backend.sql` (idempotent, versöhnt Live-DB-Drift) — Storage-Bucket-Policies, `knowledge_documents`-Umbau + Postgres-Volltextsuche (FTS/GIN, Deutsch), Join-Tabelle + RLS, Kategorien-Schreibrecht auf Admin-only, RPCs `search_knowledge_documents`/`set_document_categories`. Server Actions von In-Memory auf echtes Supabase umgestellt (Signaturen unverändert), PDF-Textextraktion via `unpdf` im Hintergrund (`after()`), Verarbeitungsstatus-Polling im Frontend. Lint/Build grün, 350/350 Unit-Tests grün, DB-Funktionen per SQL-Smoke-Test verifiziert. Interaktiver Browser-Test steht noch aus (`/qa`) | 2026-07-26 |
